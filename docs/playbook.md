# Biweekly Report Playbook — VELTRA CVR Bottleneck Analysis

> **このファイルは隔週レポート生成の唯一の運用マニュアルです。**
...
> **生成周期**: 隔週（偶数週）で実行。次回は **W16**（2026-04-19 生成予定）。
> スケジュール: W14 → W16 → W18 → W20 → ...
> 近すぎるとデータ変化が少なく施策を無理やり出す形になるため、隔週とする。
> データ集計ルール（28日ローリング等）やサマリーページの更新方法は変更なし。
...
> 生成は隔週（偶数週）月曜 AM 4:00 JST。土曜のGA4データを1日寝かせて確定させるため。
> スケジュール: W16（4/19）→ W18（5/3）→ W20（5/17）→ ...
...
**隔週（偶数週）月曜 AM 4:00 JST に実行**（土曜データを1日寝かせて確定させるため）

| 回 | 週 | 生成日（月曜） | 対象期間（日〜土） |
|----|-----|---------------|-------------------|
| 次回 | W16 | 2026-04-19 | 4/12〜4/18 |
| 次々回 | W18 | 2026-05-03 | 4/26〜5/2 |
| | W20 | 2026-05-17 | 5/10〜5/16 |
| | W22 | 2026-05-31 | 5/24〜5/30 |

> **なぜ隔週か**: 毎週だとデータの変化が小さく、施策を無理やり出す形になりがち。
> 2週間空けることで、前回施策の効果確認もでき、より質の高い分析が可能。
...
```
隔週（偶数週）月曜 04:00 JST に playbook.md を読み、Phase 1→2→3 を順次実行
```
