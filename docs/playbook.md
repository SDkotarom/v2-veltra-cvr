...
**⚠️ タイムアウト防止：過去の失敗パターンから学んだ鉄則**

> **実際に何度も `API Error: Stream idle timeout - partial response received` を引いて学んだ対策。**
> 失敗したら「リトライ」ではなく「タスク分割」で再発防止すること。

**禁止事項（必ず守る）:**

1. ❌ **サブエージェント（Agent ツール）でボトルネック content.json を埋めさせない**
   - 1ファイル分（93 TODO・HTMLモックアップ9個）を渡すと100%タイムアウトする
   - 検証済み: BN1+BN2 / BN3+BN4 / BN5+BN6 / BN7+BN8 / BN9+BN10 の5セット全て stream idle timeout
   - 代替: **メインスレッドの Edit ツールで1ファイルずつ**処理

2. ❌ **1回の Write ツール呼び出しで巨大な Python スクリプト（>300行）を出力しない**
   - `/tmp/fill_all_bottlenecks.py` のような「10ファイル一括生成スクリプト」を書こうとすると、出力途中でタイムアウト
   - 代替: **Python モジュールを <200行ごとに分割**し、複数 Write で段階的に構築

3. ❌ **1回の Edit ツール呼び出しで巨大な HTML 文字列（>3000字）を埋めない**
   - モックアップHTMLが長すぎると置換処理で詰まる
   - 代替: HTMLは **`before_text` / `after_text` 短縮形**も renderer が受け付ける（`bottleneck.html:128-129` の `wrapProtoText` 参照）。品質 vs 完走のトレードオフで、タイムアウト頻発時は短縮形を選ぶ

4. ❌ **大きな JSON ファイル全体を一度に Read しない**
   - data.json（800+行）を全部読むとコンテキスト圧迫
   - 代替: `offset`/`limit` で必要セクションだけ読む、`Grep` でセクション頭行を特定してから読む

5. ❌ **10ファイルを並列で処理しようとしない**
   - 「2〜3ファイルずつ並列 Agent」は旧版playbookの指示だが、実績として全て失敗している
   - 代替: **1ファイル = 1シーケンシャル処理** を徹底
...
# サブエージェント（Agent ツール）は使わない → stream idle timeout が発生する（上記参照）
...
直前のツール呼び出しを分析：
  - 出力サイズは？（>3000字なら分割）
  - Agent ツール経由？（メインスレッドに戻す）
  - 1回で複数ファイル？（1ファイルずつに）
  - Python スクリプトが長すぎ？（<200行に分割）
    ↓
タスクを「1操作 = 1セクション」に分割して再実行
    ↓
次回のために playbook.md に失敗パターンを追記
```
