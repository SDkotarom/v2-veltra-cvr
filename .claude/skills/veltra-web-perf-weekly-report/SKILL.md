...
## 自動更新は Apps Script が担当する

毎日の更新は `scripts/gas/perf-weekly-doc.gs`（計測スプレッドシートにバインドした Apps Script）が
固定の Google ドキュメントを上書きする形で回っている。Google サイトはそのドキュメントを
1回埋め込むだけで、以降は自動で追従する。**日次の更新でこのスキルを使う必要はない。**

本スキルを使うのは、単発で深掘りのレポートを作る場合（特定の施策の前後比較、経営向けの
まとめなど）に限る。その場合の貼り方は以下。

## Google サイトへの貼り方（単発レポートの場合）
